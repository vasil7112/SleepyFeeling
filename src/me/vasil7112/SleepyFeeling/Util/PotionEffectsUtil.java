...
public class PotionEffectsUtil {
	...
	public PotionEffectsUtil(SleepyFeeling plugin){
		this.plugin = plugin;
		for(int i=0;i< this.plugin.cConfig.getCustomConfig().getConfigurationSection("Configuration.PotionEffects").getKeys(false).size();i++){
			PotionEffect.put(i, this.plugin.cConfig.getCustomConfig().getString("Configuration.PotionEffects."+i+".Effect"));
			EffectLevel.put(i, this.plugin.cConfig.getCustomConfig().getInt("Configuration.PotionEffects."+i+".EffectLevel"));
			From.put(i, this.plugin.cConfig.getCustomConfig().getString("Configuration.PotionEffects."+i+".From"));
			To.put(i, this.plugin.cConfig.getCustomConfig().getString("Configuration.PotionEffects."+i+".To"));
		}
	}
	...
	public void addPotionEffectsToPlayer(Player player, Boolean drink){
		...
		for(int i=0;i<PotionEffect.size();i++){
			if(plugin.Energy.get(player) <= Float.valueOf(From.get(i)) && plugin.Energy.get(player) >= Float.valueOf(To.get(i))){
				if(drink){
	...
	private void getEffect(final Player player, String Effect, int i){
		if(Effect.equalsIgnoreCase("Death")){
			...
			final PlayerSleepListener PSL = new PlayerSleepListener(plugin);
	...
	private PotionEffectType getPotionEffect(String Effect){
		...
		}else if(Effect.equalsIgnoreCase("DamageResistance")){
			return PotionEffectType.DAMAGE_RESISTANCE;
		}else if(Effect.equalsIgnoreCase("FastDigging")){
			return PotionEffectType.FAST_DIGGING;
		}else if(Effect.equalsIgnoreCase("FireResistance")){
			return PotionEffectType.FIRE_RESISTANCE;
		}else if(Effect.equalsIgnoreCase("Harm")){
			return PotionEffectType.HARM;
		}else if(Effect.equalsIgnoreCase("Heal")){
			return PotionEffectType.HEAL;
		}else if(Effect.equalsIgnoreCase("HealthBoost")){
			return PotionEffectType.HEALTH_BOOST;
		}else if(Effect.equalsIgnoreCase("Hunger")){
			return PotionEffectType.HUNGER;
		}else if(Effect.equalsIgnoreCase("IncreaseDamage")){
			return PotionEffectType.INCREASE_DAMAGE;
		}else if(Effect.equalsIgnoreCase("Invisibility")){
			return PotionEffectType.INVISIBILITY;
		}else if(Effect.equalsIgnoreCase("Jump")){
			return PotionEffectType.JUMP;
		}else if(Effect.equalsIgnoreCase("NightVision")){
		...
		}else if(Effect.equalsIgnoreCase("SlowDigging")){
			return PotionEffectType.SLOW_DIGGING;
		}else if(Effect.equalsIgnoreCase("Speed")){
			return PotionEffectType.SPEED;
		}else if(Effect.equalsIgnoreCase("WaterBreathing")){
